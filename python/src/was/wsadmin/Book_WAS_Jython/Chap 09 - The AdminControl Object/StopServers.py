...
#  ISBN: 0-13-700952-6
#  Role: Script used to stop all managed application servers in the
#        cell.
# Usage: wsadmin -lang jython -f StopServers.py
...
#---------------------------------------------------------------------
def StopServers( cmdName = 'StopServers' ) :
  #-------------------------------------------------------------------
  # Name: oops()
  ...
  try :
    ...
    if pType == 'DeploymentManager' :
      ...
      # servers within each node, and try to stop each
      #-----------------------------------------------------------------
...
def Usage( cmdName ) :
  print '''
Command: %(cmdName)s\n
Purpose: WebSphere (wsadmin) script used to stop all "managed" Application
